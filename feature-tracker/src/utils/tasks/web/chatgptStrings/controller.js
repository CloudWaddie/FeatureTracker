...
export default async function chatgptStringsController() {
    ...
    try {
        ...
        // --- Move browser.close() here inside the try block ---
        if (browser) {
            await browser.close();
        }
        // --- End of browser.close() move ---
        ...
        try {
            ...
            if (deletions.length > 0) {
                if (formattedDetails.length > 0) formattedDetails += '; ';
        ...
        // Only close browser here if it's still open due to an error that
        // prevented the earlier close (e.g., if it failed to launch initially)
        if (browser && !browser.isConnected()) { // Check if it's connected before trying to close again
             await browser.close();
        }
    }
